...
| **(parallel)** | Primers + thermodynamics (Tm/GC → display → design) | 🟡 Phase 0.1 done (thermo + `seqforge tm`) |
...
| **Primers + thermo** | [`plans/primers.md`](plans/primers.md) | 🟡 Phase 0.1 done (`seqforge-thermo` vendored + `seqforge tm`); design settled beyond | Phase 0.2 — `core`: `Primer` + `PrimerId` + `Annotations.primers` |
