...
| **v0.3** | **Cloning / assembly** — PCR → fragments → ligation/Golden Gate, fidelity-scored, CLI/GUI/agent parity, batch | ⏳ next milestone (design of record complete) |
...
| **Assembly** | [`plans/assembly.md`](plans/assembly.md) | 📋 design of record | A1 — fragment sources + recipe document (Tab); `Fragment` = the transport carrier + `End`s (decision 23) |
| **Fidelity** | [`plans/fidelity.md`](plans/fidelity.md) | 📋 design of record | F1 — Potapov data pipeline (⚠ license gate first) |
...
| 22 | **`seqforge-fidelity` = the methylation pipeline again** (decision 18): gitignored Potapov snapshot → `codegen` → committed `fidelity_generated.rs` static matrix, zero-dep/extractable; `junction_fidelity(overhangs)` is the read-side metric, `suggest_set`/`split_target` the GetSet/SplitSet analogs. **Blocked on a data-license check before any matrix is vendored.** | One pipeline pattern across REBASE / methylation / fidelity keeps engine crates pure and reviewable; the license gate is real (NEB *tools* ≠ published *data* terms) so it's called out as the Tier-1 blocker, with GoldenHinges / fetch-at-ingest as fallbacks | [`plans/fidelity.md`](plans/fidelity.md) |
