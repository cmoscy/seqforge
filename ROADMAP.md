...
| **(parallel)** | Primers + thermodynamics (Tm/GC → display → design) | 🟡 Phase 0.1/0.5/0.2 done (thermo, live Tm/GC, `Primer` model) |
...
| **Primers + thermo** | [`plans/primers.md`](plans/primers.md) | 🟡 Phase 0.1/0.5/0.2 done (thermo + `seqforge tm`; live Tm/GC readout; `Primer` core model + shift handler) | Phase 0.3 — GenBank `primer_bind` ↔ `Primer` round-trip |
